...
class Base():
    ...
    def get_headers(self):
        """
        处理请求头
        :return:返回的是字典格式的请求头，多是包括了Content-Ty，X-Litemall-Admin-Token
        """
    ...
    def login(self):
        """
        通过调用登录接口获取token值，将其进行缓存，其他接口使用时直接从缓存中取
        若没有去取到，再调用登录，再将token值放在缓存中
        :return:
        """
        login_path = '/admin/auth/login'
        login_url = self.get_url(login_path)

        self.post(login_url,LOGIN_INFO) # 请求登录接口 返回json数据
